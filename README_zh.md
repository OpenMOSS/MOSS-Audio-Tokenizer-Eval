...
推荐使用项目提供的单环境配置。该环境包含模型重建、数据准备、默认指标和结果汇总所需的依赖：
...
这里固定 `pip=24.0` 是为了兼容 `omegaconf==2.0.6` 的依赖元数据；`env -u CUDA_HOME` 用于避免 `fairseq` 在新版 PyTorch/CUDA 环境下编译旧 CUDA 扩展。`requirements-optional.txt` 会安装 `sim` 和 `utmos` 所需的 Python 包，相关权重文件仍需按下文说明单独下载。

如果希望安装过程显示 pip 下载进度，也可以用等价的默认依赖拆分方式：
...
| `sim` | 16 kHz / mono speaker similarity | 安装 `requirements-optional.txt`；准备 `wavlm_large_finetune.pth` |
...
`utmos` 按需加入 `metrics.enabled`。可选依赖安装方式见上文 `## 安装`。
