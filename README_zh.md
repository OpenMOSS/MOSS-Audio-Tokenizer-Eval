...
项目默认走一个 Conda/Python 环境完成：模型重建、常用客观指标、manifest、结果汇总都通过同一个 CLI 管理。对于需要额外环境或权重的指标，可以在配置里显式指定本地路径。
...
推荐使用项目提供的单环境配置。该文件会安装重建评测、默认指标和 `utmos` 依赖所需的 Python 包：
...
如果你已经有兼容的 PyTorch 环境，也可以直接安装同一套 pip 依赖。由于 UTMOS 依赖的 `omegaconf==2.0.6` 需要 `pip<24.1`，直接用 pip 安装前请先确认当前 pip 版本：
...
| `stft` | STFT distance | 不需要 |
| `sim` | 16 kHz / mono speaker similarity | 需要 `wavlm_large_finetune.pth` |
| `utmos` | 调用 UTMOS 外部脚本；评测前临时转 `24 kHz / mono` | 需要 UTMOS 环境、`epoch=3-step=7459.ckpt`、`wav2vec_small.pt` |
...
`utmos` 需要额外权重，按需加入 `metrics.enabled`。该指标通过外部评测工程调用：

```bash
python do_reconstruct_evalation_given_dir.py --output_dir <dataset_output_dir>
```

调用外部脚本前，本项目会先临时生成 `24 kHz / mono` 的 `gt_audios`、`syn_audios`，再交给外部脚本评测；原始重建音频不会被改动。
...
UTMOS 依赖已经写入默认的 `environment.yml` 和 `requirements-eval.txt`，正常安装项目环境后不需要额外安装单独的 UTMOS requirements。

已在该环境中验证过 UTMOS checkpoint 可以正常加载和评分。当前 CLI 调用 `utmos` 时会自动设置 `TORCH_FORCE_NO_WEIGHTS_ONLY_LOAD=1`，以兼容 PyTorch 2.6+ 的 checkpoint 加载行为；如果手动进入 UTMOS 目录直接运行它自己的脚本，需要自行设置这个环境变量。
...
下载后放到 UTMOS work_dir 根目录。换机器或离线运行时，需要同时准备 UTMOS work_dir 里的评测脚本、上述两个权重文件，以及可运行的 `utmos` Python/Conda 环境。
